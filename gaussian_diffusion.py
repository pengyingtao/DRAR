...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
        ...
        self.noise_scale = noise_scale  # 0.1 
        self.noise_min = noise_min  # 0.0001
        self.noise_max = noise_max  # 0.2
        self.steps = steps  # 100
        self.device = device # 

        self.history_num_per_term = history_num_per_term  # 10
        self.Lt_history = th.zeros(steps, history_num_per_term, dtype=th.float64).to(device) # torch.Size([100, 10])
        self.Lt_count = th.zeros(steps, dtype=int).to(device)  # torch.Size([100])
    ...
    def calculate_for_diffusion(self):   # 加噪
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        # print('ts', ts.shape)  # ts torch.Size([400])  # ts 按权重的step的顺序
        # print('pt', pt.shape)  # pt torch.Size([400])  # pt 表示所有的权重值
        
        noise = th.randn_like(x_start)  # 生成随机噪声 torch.Size([400, 2810])
        ...
        terms = {} # loss的存放
        model_output = model(x_t, ts)  # 模型的输出，这里是预测的noise
    ...
    def sample_timesteps(self, batch_size, device, method='uniform', uniform_prob=0.001):
        if method == 'importance':  # importance sampling
            ...
            Lt_sqrt = th.sqrt(th.mean(self.Lt_history ** 2, axis=-1))   # torch.Size([100])
            pt_all = Lt_sqrt / th.sum(Lt_sqrt)
            pt_all *= 1- uniform_prob
            pt_all += uniform_prob / len(pt_all)  # torch.Size([100])

            assert pt_all.sum(-1) - 1. < 1e-5
            # 作用是对input的每一行做n_samples次取值，输出的张量是每一次取值时input张量对应行的下标。 按照pt_all的权重大小顺序 取值
            t = th.multinomial(pt_all, num_samples=batch_size, replacement=True)
            pt = pt_all.gather(dim=0, index=t) * len(pt_all)
            return t, pt
        
        elif method == 'uniform':  # uniform sampling
            t = th.randint(0, self.steps, (batch_size,), device=device).long()  # 随机step的序号
            pt = th.ones_like(t).float()  # pt 的所有值为1
            return t, pt
